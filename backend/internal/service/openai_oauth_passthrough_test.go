...
func TestOpenAIGatewayService_OAuthPassthrough_CompactPreservesOfficialDesktopVersionAndUserAgent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5.5","stream":true,"store":true,"input":[{"type":"text","text":"compact me"}]}`)

	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid-compact-desktop"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"cmp_555","usage":{"input_tokens":11,"output_tokens":22}}`)),
	}
	upstream := &httpUpstreamRecorder{resp: resp}
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "0.125.0-alpha.3", upstream.lastReq.Header.Get("Version"))
	require.Equal(t, "Codex Desktop/0.125.0-alpha.3 (Mac OS 26.2.0; arm64)", upstream.lastReq.Header.Get("User-Agent"))
	require.Equal(t, "message", gjson.GetBytes(upstream.lastBody, "input.0.type").String())
	require.Equal(t, "input_text", gjson.GetBytes(upstream.lastBody, "input.0.content.0.type").String())
}
